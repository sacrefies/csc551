    ...
    for (int i = 0; i < argc; i++) {
        ...
        if (strcmp("-s", argv[i]) == 0 || strcmp("--sort", argv[i]) == 0) {
            if (strcmp("insertionSort", argv[i + 1]) == 0)
                ...
                options.sort = 2;
            else if (strcmp("quickSort", argv[i + 1]) == 0)
                options.sort = 3;
            else {
                cout << "Sort " << argv[i + 1] << " is not supported yet.";
                cout << endl;
                cout << "The default quick sort will be applied." << endl;
            }
            continue;
        }
...
int main(const int argc, const char *argv[]) {
    ...
    switch (options.sort) {
        ...
        case 2:
            ...
            break;
        case 3:
            timing(options.runs, options.size, quickSort);
            break;
        default:
